...
export default function CartPage() {
    const { cartItems, incrementQuantity, decrementQuantity, removeFromCart, clearCart, total } = useCart();
    const { currentUser } = useAuth();
    const router = useRouter();

const handleCheckout = () => {
    if (!currentUser) {
        toast.error("Iniciá sesión para completar la compra");
        router.push("/login");
        return;
    }
...
return (
    ...
    <div className="border-t border-gray-800 pt-4 flex items-center justify-between">
        ...
        <button
            ...
            className="bg-cyan-400 text-black font-medium px-6 py-2.5 rounded-md hover:bg-cyan-300 hover:shadow-[0_0_10px_rgba(34,211,238,0.5)] transition-all"
        >
            Proceder al pago
        </button>
    </div>
